    ...
    <div className="space-y-10">
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950 p-5 shadow-[0_0_0_1px_rgba(16,185,129,0.08)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400">
              Live <span className="text-slate-200">gold price today</span>,{" "}
              <span className="text-slate-200">silver spot price</span>, and{" "}
              <span className="text-slate-200">platinum spot price</span> shown as USD per troy ounce
              of <span className="text-slate-200">pure</span> metal. For jewelry and items, use the{" "}
              <Link href="/gold" className="text-emerald-300 hover:underline">
                gold calculator
              </Link>{" "}
              to estimate melt value by karat/purity and weight.
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {[
            { label: "Gold spot", value: spot?.gold, sub: "USD per troy oz (pure)" },
            { label: "Silver spot", value: spot?.silver, sub: "USD per troy oz (pure)" },
            { label: "Platinum spot", value: spot?.platinum, sub: "USD per troy oz (pure)" },
          ].map((card) => (
            <div
              key={card.label}
        ...
        {error ? (
          <div className="mt-4 rounded-xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-100">
            ...
            <div className="mt-2 text-xs text-rose-100/70">
              If the feed is temporarily unavailable, the page will show dashes. You can retry with
              “Refresh prices.”
            </div>
          </div>
        ) : null}
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Estimated <span className="text-slate-200">gold price per gram</span> and{" "}
              <span className="text-slate-200">gold price by karat</span> (10K, 14K, 18K, 22K, 24K),
              calculated as spot × purity. This reflects raw metal content only — real offers can be
            ...
            <tbody className="divide-y divide-slate-800 bg-slate-950">
              {rows.map((r) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-slate-100">
                    {r.spot > 0 ? formatMoney(r.perOzEquivalent) : "—"}
                  </td>
        ...
        <div className="mt-4 text-xs text-slate-400 leading-relaxed">
          Tip: If you’re comparing your number to a quote, make sure you’re using the same unit.
          Spot is typically shown per <span className="text-slate-200">troy ounce</span> (31.1035g),
          not a standard avoirdupois ounce (28.35g).
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400">
            Enter the metal, karat/purity, and weight to estimate melt value using the live spot
            prices above. For the full experience, go to{" "}
            <Link href="/gold" className="text-emerald-300 hover:underline">
              /gold
            </Link>
            .
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-950 p-5">
        <h2 className="text-lg font-semibold text-slate-50">
          How These Prices Are Calculated (Spot → Gram → Karat)
        </h2>

        <div className="mt-3 space-y-3 text-sm text-slate-300 leading-relaxed">
          <p>
            <span className="text-slate-100">Spot price</span> is quoted in USD per{" "}
            <span className="text-slate-100">troy ounce</span> of{" "}
            <span className="text-slate-100">pure</span> metal.{" "}
            <span className="text-slate-100">Price per gram</span> converts that spot price to grams
            (1 troy oz = 31.1035g).{" "}
            <span className="text-slate-100">Gold price by karat</span> estimates the value of the
            gold content by applying purity (example: 14K gold is 14/24 = 58.3%).
          </p>

          <p>
            These are <span className="text-slate-100">reference estimates</span> for metal content.
            They do <span className="text-slate-100">not</span> include brand value, gemstones,
            craftsmanship, sales tax, shipping, or marketplace fees.
          </p>

          <p>
            If you’re selling, offers are often lower than melt value because buyers factor in
            verification/testing, refining costs, risk, and margin. If you want a fast, item-level
            estimate, use the{" "}
            <Link href="/gold" className="text-emerald-300 hover:underline">
              gold calculator
            </Link>{" "}
            with your item’s weight and karat.
          </p>
        </div>
      </section>
    </div>
