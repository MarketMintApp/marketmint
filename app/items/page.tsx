              ...
              {visibleValuations.map((v) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      setExpandedId((current) => (current === v.id ? null : v.id))
                    }
                  >
                    {/* Row main content */}
                    <div className="py-3">
                    ...
                    <div className="relative py-3 text-right space-y-1">
                      <div className="inline-flex items-center justify-end gap-1">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setOpenMenuId((current) => (current === v.id ? null : v.id));
                          }}
                    ...
                    <div
                      className={`col-span-5 overflow-hidden border-t border-slate-800/80 bg-slate-950/80 transition-all duration-300 ease-out ${
                        isExpanded ? "max-h-64 py-3 opacity-100" : "max-h-0 py-0 opacity-0"
                      }`}
                    >
                      {offersForVal.length > 0 ? (
                        <div className="space-y-2 text-[11px] text-slate-300">
                          <p className="font-semibold text-slate-200">Offers for this item</p>
                          ...
                          {[...offersForVal]
                            ...
                            .map((o) => {
                              const pct = melt > 0 ? (o.amount / melt) * 100 : null;
                              const isBest = best && best.offer.id === o.id;
                              const chipClass = getOfferStatusChipClass(o.status);
                        ...
                        <p className="text-[11px] text-slate-400">
                          ...
                          <span className="font-medium">Get offers</span> to start tracking buyer quotes for this item.
                        </p>
                      )}
                    </div>
                  </div>
